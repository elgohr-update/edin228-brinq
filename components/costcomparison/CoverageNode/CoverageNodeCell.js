...
export default function CoverageNodeCell({nodeCell,rowId,nodeId}) {
    ...
    const dataStyle = `relative border ${isDark? `border-gray-800`:``} text-xs w-[200px] py-1 flex items-center justify-end text-right pr-2 pl-6`
            ...
            editMenu ? 
                <div className="relative z-40">
                    <div className={`flex items-center absolute w-auto space-x-2 z-40 right-[-10px] p-2 rounded-lg top-[-45px] panel-flatter-${type} ${type}-shadow`}> 
                        ...
                        <div className="flex items-center b-button-group space-x-1">
                    ...
                    <div>
                       <Input onChange={ (e) => setText(e.target.value)} autoFocus size="xs" initialValue={nodeCell.data} clearable underlined/> 
                    </div>
                </div> 
                : 
                <div className={`${nodeCell.textBold?'font-bold':''} ${nodeCell.textItalic?'italic':''}`} onClick={() => showCellEdit()}>
                    {
                    nodeCell.isMoney && !isNaN(nodeCell.data)? 
                        `$${formatMoney(Number(nodeCell.data))}`: 
                    nodeCell.data.length < 1 ? 
                        <div className="h-[10px] min-w-[100px]">{nodeCell.data}</div> : 
                    nodeCell.data
                    }
                </div>
            }

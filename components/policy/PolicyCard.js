...
const PolicyCard = ({policy,border=false,truncate=20,vertical=false,shadow=false}) => {
    ...
    const baseClass = `relative text-xs transition-all ${border?`${isDark?`border-slate-900`:`border-slate-200`} border`:null} ${shadow?`${type}-shadow`:null} duration-200 ease-out flex-col w-full p-2 rounded-lg ${isBorder()} ${isVertical()} ${isPanel()} ${isShadow()}`
    return (
